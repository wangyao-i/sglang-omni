...
def repo(tmp_path: Path) -> Path:
    ...
    fake_python.write_text(
        ...
        "  printf '%s\\n' \"${FAKE_SGLANG_VERSION:-0.5.19}\"\n"
        'elif [[ "$1" == "-" && $# -ge 2 ]]; then\n'
        '  case "$2" in\n'
        "    0.5.19|0.5.19.*|0.5.19+*|0.5.19a*|0.5.19b*|0.5.19rc*) exit 0 ;;\n"
        "    *) exit 1 ;;\n"
        "  esac\n"
        'elif [[ "$1" == "-c" ]]; then\n'
        "  printf '%s\\n' \"$0\"\n"
...
def test_mismatched_sglang_version_is_rejected(repo: Path, installed: str) -> None:
    ...
    assert result.returncode != 0
    assert "supported: 0.5.19 release line" in result.stderr
