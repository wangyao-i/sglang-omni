...
| Combined NPU liveness and correctness | Partial | Pure `v0.5.19` + Omni `5190678c` passed graph-only startup, smoke, and the 70-request cold concurrency-8 gate; a first correctness attempt used the known non-equivalent exact10 concurrency-1 protocol and WER `0.0784`, so it is not accepted | Confirm cleanup, then run the 140-request concurrency-8 correctness gate in [`qwen3_asr_ascend_v0519_liveness_correctness_task.md`](qwen3_asr_ascend_v0519_liveness_correctness_task.md) |
