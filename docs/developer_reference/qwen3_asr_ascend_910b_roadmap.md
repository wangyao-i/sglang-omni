...
| SGLang-Omni all-graph/realtime integration code | `f55c3b094419b4b8c2aba84d83c1c55c0ebaa1de`; combines PR #2016, the validated PR #2160 bucket-key update path, the NPU graph-only profile, bounded Qwen3-ASR final-prefix behavior, the PR #2016 stability-gate correction, and the full-suite test-builder identity fix |
...
- The all-graph implementation has changed since the earlier smoke and
  ...
  correctness workload on integration code head `f55c3b09`.
...
- one cold concurrency-8 run over 140 requests succeeds on `f55c3b09` with no
