...
|---|---|
| SGLang-Omni | `upstream/main@886ced95` contains merged #2084; the active follow-up runtime code is `4e72cf2e` |
...
| NPU encoder graph | Implemented | Follow-up runtime code `4e72cf2e` lazily captures exact NPU window signatures with fair admission inside an explicit global capacity | Pass the pinned all-graph hardware task with zero fallback markers |
...
runtime code `4e72cf2e`. No compile-supported NPU claim and no fused-op
