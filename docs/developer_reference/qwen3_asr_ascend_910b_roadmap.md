...
| Combined NPU liveness and correctness | Planned | Pure `v0.5.19` + Omni `5190678c` passed graph-only startup and one smoke request; the next task is [`qwen3_asr_ascend_v0519_liveness_correctness_task.md`](qwen3_asr_ascend_v0519_liveness_correctness_task.md) | Run cold concurrency-8 liveness and the 140-request correctness workload on the exact pure-base heads |
