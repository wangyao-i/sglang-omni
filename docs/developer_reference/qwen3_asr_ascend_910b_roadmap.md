...
|---|---|
| SGLang-Omni | `upstream/main@886ced95` contains merged #2084; the active follow-up runtime code is `2cdfa51e` |
...
| NPU encoder graph | Implemented | Follow-up runtime code `2cdfa51e` lazily captures exact NPU window signatures and bounds the global signature cache by `max_batch_size` | Pass the pinned all-graph hardware task with zero fallback markers |
...
runtime code `2cdfa51e`. No compile-supported NPU claim and no fused-op
