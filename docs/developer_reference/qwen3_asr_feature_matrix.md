...
| Prefill/decode graph | Not a model contract | Prefill uses the breakable backend; decode graph is enabled by default | Engine builder and graph tests | SGLang v0.5.19 NPU graph path | Graph-only with compile disabled passes readiness, decode capture, one non-empty smoke, and shutdown on exact heads |
