...
| Encoder graph | Not a model contract | Optional platform-owned encoder layer-stack graph | CUDA graph tests | NPU adapter on `4e72cf2e` lazily captures exact `(bucket_size, window_lens)` signatures with fair admission inside one explicit global capacity | All-graph NPU qualification pending |
