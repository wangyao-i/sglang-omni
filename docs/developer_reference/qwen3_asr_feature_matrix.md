...
| Encoder graph | Not a model contract | Optional platform-owned encoder layer-stack graph | CUDA graph tests | NPU adapter on `8ad2a5a9` mirrors the ViT NPU graph runner: lazy exact-layout capture with host-resident boundaries | All-graph NPU qualification pending |
