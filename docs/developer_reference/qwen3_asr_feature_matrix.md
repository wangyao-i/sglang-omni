...
| Torch compile | Not a model contract | Enabled by default in the CUDA performance profile with `torch_compile_max_bs=2`; the stage default is disabled | Engine builder/default config | CUDA performance option; not required by the Qwen3-ASR model | Compile+decode-graph startup failed; graph-only is the next functional gate |
