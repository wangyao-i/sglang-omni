...
| 910C-059 | Omni `c49e1ae5`; SGLang `e4d18390a`; no server edit | Replace the NPU backend's Python update-thread/join overlap with opt-in same-thread ordered `NPUGraph.update()` then `replay()` | Serial focused/full tests; Arm O with qualified forward guard; conditional fresh graph-scope arm under the fixed M1c profile | liveness passed; accuracy/performance summary incomplete | Tests passed (SGLang 16; Omni guard 4, pipeline 42, Qwen3-ASR 606 with 3 skipped). Arm O completed cold conc8 70/70, correctness 140/140, and C70 700/700; 5,083 ordered update/replay pairs completed, no update-thread markers, final guard outstanding 0. Arm G completed correctness 140/140 and C70 700/700; 4,706 pairs completed, no update-thread markers, final guard outstanding 0. This closes the prior join-based liveness failure, but the returned text omitted WER, garbled count, p95/p99, throughput/RTFx, graph counters, and complete per-label guard statistics, so graph-scope performance benefit is not yet established. |
| 910C-059R | no code or hardware change; retained `910C-059` artifacts only | Complete the predeclared accuracy and performance result without rerunning either arm | Read-only extraction from Arm O/G result JSON, raw JSONL, event JSONL, model-info, poller, and logs | authorized; pending text return | Return WER, garbled/failed/timeout/missing/duplicate/unexpected counts, p95/p99/max, throughput, RTFx, graph capture/replay/fallback counters, and per-label guard wait/acquire/release plus wait/hold maxima for both arms. Also return graceful cleanup/HBM state. Do not start a service or edit anything. |
