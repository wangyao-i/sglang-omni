...
| 910C-064 | handoff commit containing this row; SGLang `e4d18390a`; Omni branch `qwen3-asr-guard-scope`; no server edit | Test guard-off liveness at the profile that originally hung, which `910C-063` did not cover | Three fresh `ordered + scope=off` processes each executing the explicit cold concurrency-8 gate at the `910C-013` profile, then one fourth-process 140 correctness and exact10 C70 | authorized after a clean idle NPU baseline; pending | Profile fixed at the `910C-013` point: compile disabled, encoder graph disabled, prefill graph disabled, decode graph captured through bucket 70, eight request-build workers, pinned 70-input selection, concurrency 8 with no warm-up, and a 120 s request timeout. Require the guard-disabled warning, a model-info attestation that no guard is installed, and empty per-label guard statistics. Do not treat an implicit pass as a gate; stop on the first timeout, 90 seconds without a completion, accuracy or output failure, OOM, device error, state leak, orphan, or cleanup failure, and do not retry with the guard re-enabled. Any hang means the guard is load-bearing and the review request to drop or condition it is declined with that evidence; three clean gates plus a clean C70 mean the trigger no longer reproduces and the next task is a narrow-barrier variant, not shipping the `off` switch. |
