...
| 910C-057 | Omni candidate `qwen3-asr-guard-scope` at the handoff commit containing this row (code through `f24d6220`); SGLang candidate `qwen3-asr-perf-next` through `b03400e5a` | Independently remove the stateful decode-attention compile break and narrow the FIFO guard from the whole generation forward to exact graph dispatch | Serial AC explicit-state attention, GS graph-scoped guard, and conditional AG composition; each has correctness before C70 performance | authorized with fixed M1c `[1,2,70]` baseline | Both changes are opt-in and default-safe. AC makes ForwardBatch/KV tensor state and mutation explicit to Dynamo. GS retains encoder serialization but guards only prefill/decode graph execute and exposes labeled wait/hold totals. Cold-input deadlock, zero eager/fallback, WER, request accounting, balanced guard, and cleanup are hard gates. |
