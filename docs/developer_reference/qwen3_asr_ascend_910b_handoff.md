...
| 910C-065 | handoff commit containing this row; SGLang `e4d18390a`; Omni branch `qwen3-asr-guard-scope`; no server edit | Counterfactual that explains the original hang: guard-off under the threaded update path, where the decode graph contributes its own update helper thread | Three fresh `threaded + scope=off` processes each executing the explicit cold concurrency-8 gate at the `910C-013` profile, then one fourth-process 140 correctness and exact10 C70 | authorized after a clean idle NPU baseline; pending; a hang is the predeclared expectation, not a fault | Same `910C-013` profile as `910C-064`: compile disabled, encoder graph disabled, prefill graph disabled, decode graph captured through bucket 70, eight request-build workers, pinned 70-input selection, concurrency 8 with no warm-up, 120 s request timeout; the only change from `910C-064` is `SGLANG_NPU_GRAPH_INPUT_UPDATE_MODE=threaded`. Bounded forced cleanup is explicitly authorized because a hung service may not stop gracefully: report residual HBM per chip, any remaining holder and whether it is a user-space process, port state, and the cleanup sequence used, and do not start the next process until the device is at baseline or the residual state is recorded. Do not retry with the guard re-enabled or with ordered mode. A hang confirms the threaded helper as the third concurrent submitter, so the guard becomes redundant once ordered update is adopted and the candidate can ship without it; three clean gates plus a clean C70 mean the hazard is not the threaded helper, and the next task is the narrow-barrier variant. The `off` scope is not shipped either way. |
