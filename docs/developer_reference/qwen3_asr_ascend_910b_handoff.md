...
| 910C-059 | Omni `99aabb26`; SGLang `e4d18390a`; no server edit | Replace the NPU backend's Python update-thread/join overlap with opt-in same-thread ordered `NPUGraph.update()` then `replay()` | Serial focused/full tests; Arm O with qualified forward guard; conditional fresh graph-scope arm under the fixed M1c profile | ordered liveness passed; graph scope not adjudicated | Tests passed (SGLang 16; Omni guard 4, pipeline 42, Qwen3-ASR 606 with 3 skipped). Arm O completed cold conc8 70/70, correctness 140/140, and C70 700/700; C70 throughput was 52.28/s, 5,083 ordered pairs completed, update-thread markers were zero, and guard outstanding was zero. Arm G also drained at 51.66/s with 4,706 complete ordered pairs and left HBM at 22%. Arm G's 140-request WER of 0.0778 is withdrawn as an accuracy basis: `910C-062` measured the same gate on the qualified harness at 0.0176 (threaded) and 0.0179 (ordered) with raw per-sample output retained, and `910C-063` returned 0.0179, so the earlier reading is not comparable evidence. The 1.81 s / 51.66/s comparison against M1c is equally inadmissible because it came from the same non-equivalent protocol. Graph scope is therefore not adjudicated on evidence; it is closed only for absence of benefit, since the entire guard is worth a few percent. Retain the broad `forward` scope. |
| 910C-059R | no code or hardware change; retained `910C-059` artifacts only | Complete the predeclared accuracy and performance result without rerunning either arm | Read-only extraction from Arm O/G result JSON, raw JSONL, event JSONL, model-info, poller, and logs | completed sufficiently for rejection | Arm G returned zero garbled output but WER 0.0778, p95 1.81 s, p99 2.01 s, max 2.45 s, and 51.66/s. Generation graph holds were bounded in milliseconds, but encoder hold max remained 12.641 s. Missing Arm O accuracy/latency and graph-counter fields prevent promotion of ordered-forward mode; they do not prevent rejecting graph scope. |
| 910C-060 | SGLang `e4d18390a`; Omni `21e755be`; no server edit | Test the only remaining coherent narrowed boundary after ordered update removed the prior model-scope join stall | One fresh `ordered + scope=model` process using the fixed M1c profile; 140 correctness then conditional exact10 C70 | live and deadlock-free; its accuracy and performance comparisons withdrawn | Correctness 140/140, zero garbled output, 4,707 ordered update/replay pairs, zero update-thread markers, final guard `next=serving=5324, outstanding=0`; teardown HBM was not confirmed. Its 140-request WER of 0.0778 and its C70 p95 1.75 s / 52.79 req/s are no longer evidence against model scope: `910C-062` measured the same gate on the qualified harness at 0.0176 and 0.0179 with retained raw output, and the earlier readings share the non-equivalent protocol of `910C-061`. Model scope is not adjudicated on evidence and is closed only for absence of benefit. Retain the broad `forward` scope. |
| 910C-061 | SGLang `e4d18390a`; Omni `99aabb26`, code-equivalent to `21e755be` (the diff is four documentation files); no server edit | Qualify ordered graph input update under the retained `forward` guard, which no accuracy-bearing run had measured | Read-only 059 Arm O field check, then one fresh `ordered + scope=forward` process at the fixed M1c profile | liveness passed; accuracy and performance not attributable | Ordered update under `forward` was live at C70: 700/700, 4,709 complete ordered update/replay pairs, zero `update_thread_*` markers, final guard `next=serving=5334, outstanding=0`, 140/140 with zero garbled, cleanup to 5% HBM. The 140-request WER was 0.0778, the same value that rejected graph scope in `910C-059`, and the C70 WER was unscorable because the bespoke aiohttp client persisted no raw hypotheses. The C70 comparison to M1c is inadmissible: four protocol variables changed at once (bespoke client, no warm-up, 140 gate at concurrency 1, single measurement). The M1c references are the 140-request gate at WER 0.0161 and C70 at WER 0.0164, p95 1.442 s, 59.88 req/s. `910C-062` replaces the comparison with a single-variable A/B. |
| 910C-062 | handoff commit containing this row; SGLang `e4d18390a`; Omni branch `qwen3-asr-guard-scope`; no server edit | Attribute the 140-request WER discrepancy and compare ordered versus threaded graph input update under one protocol | Read-only `910C-056A`/`056B` and `910C-061` protocol forensics, then two arms differing only in `SGLANG_NPU_GRAPH_INPUT_UPDATE_MODE`, each with cold conc8, 140 correctness, and three fresh-process C70 repeats | authorized after a clean idle NPU baseline; pending | Arm A `threaded` + `forward`, then arm B `ordered` + `forward`; packaged exact10 harness only; identical corpus manifest SHA across arms; raw per-sample JSONL for both phases in both arms, since unscorable raw output fails the run; always-on guard/encoder/prefill/decode counters on every C70; Dynamo break/recompile logging on the first C70 of each arm only; never a narrowed guard scope. Adjudicate the 140-request WER with the three-outcome table in the task document and performance only against each arm's own three-measurement band. Do not retry with another scope, ordering, token cap, bucket set, or memory fraction. |
| 910C-063 | SGLang `e4d18390a`; Omni branch `qwen3-asr-guard-scope`; no server edit | Discriminate whether the NPU execution guard is load-bearing or only orders two device producers, following the `vllm-ascend` `#4233` precedent | One fresh `ordered + scope=off` process at the fixed M1c profile: batch one, cold conc8, 140 correctness, exact10 C70 | completed once with no hang; not sufficient to drop the guard | Guard-off at the fully accelerated profile completed C70 700/700 at WER 0.0161, p95 1.478 s, and 57.14 req/s, with graceful shutdown and idle HBM; the 140-request gate read 0.0179. This does not cover the configuration where the guard was introduced: `910C-013` hung with the encoder graph disabled, prefill graph disabled, compile disabled, decode graph enabled through bucket 70, eight request-build workers, and concurrency 8, at server commit `9bae2619`, which predates the guard commit `ef6db56c`; `910C-024B` later passed that same family with the guard enabled. The failure mode is intermittent and this was a single run whose cold concurrency-8 gate was implicit, so a pass is weak evidence while a hang would be strong. The guard costs a few percent, not the throughput gap. `910C-064` tests guard-off at the historical failure profile. |
| 910C-064 | SGLang `e4d18390a`; Omni branch `qwen3-asr-guard-scope`; no server edit | Test guard-off liveness at the profile that originally hung, which `910C-063` did not cover | Three fresh `ordered + scope=off` processes each executing the explicit cold concurrency-8 gate at the `910C-013` profile, then one fourth-process 140 correctness and exact10 C70 | passed; the historical trigger does not reproduce under ordered mode | Three gates passed 70/70 with zero failures and zero timeouts at p95 13.26-14.87 s and 3.6-3.9 requests/s; the fourth process read 0.0179 at the 140 gate and 700/700 at C70 with WER 0.0162, p95 1.716 s, and 50.5 requests/s. Guard-disabled warning, no guard installed in model-info, empty per-label guard statistics, free port, and baseline HBM in every process. Caveat: `910C-013` predates ordered input update, so it ran the threaded path, where the decode graph itself submits from two host threads (`replay()` on the forward thread, `graph.update()` on a helper thread); adding the encoder thread makes three concurrent submitters, which matches the `910C-020` stall at `update_thread_join`. This arm therefore shows the guard is unnecessary while ordered mode is in force, not that it is unnecessary in general. `910C-065` tests the threaded counterfactual. |
| 910C-065 | SGLang `e4d18390a`; Omni branch `qwen3-asr-guard-scope`; no server edit | Counterfactual that explains the original hang: guard-off under the threaded update path, where the decode graph contributes its own update helper thread | Three fresh `threaded + scope=off` processes each executing the explicit cold concurrency-8 gate at the `910C-013` profile, then one fourth-process 140 correctness and exact10 C70 | hung at Gate 1, as predeclared; mechanism confirmed | Marker counts: `update_thread_enter` 37, `update_thread_join_begin` 37 with 36 returns, `graph_update_begin` 37 with 36 returns; the last event was `update_thread_join_begin key_size=4`. One iteration entered the join and never returned and its `graph.update()` never returned, reproducing the `910C-020` signature with the encoder thread present. Under the threaded path the forward thread waits on the join, the helper thread is stuck in `graph.update()`, and the encoder is the third concurrent submitter; ordered update removes the helper, and `910C-064` shows the same profile is live without a guard once only two submitters remain. The guard's original justification is superseded rather than wrong. `910C-066` tests whether a narrow barrier can replace the mutex at this same hanging bench. |
| 910C-066 | handoff commit containing this row; SGLang `e4d18390a`; Omni branch `qwen3-asr-guard-scope`; no server edit | Decide whether a device barrier replaces the FIFO mutex, tested where the hazard exists rather than at a profile that never hangs | Barrier-only mode: `scope=off` plus `SGLANG_OMNI_NPU_GUARD_COMPLETION_FENCE` builds `FairDeviceExecutionGuard(serialize=False)`, bypassing the ticket lock while the existing full-device fence runs at every handoff. One fresh process at the `910C-013` profile with `threaded` update, running the explicit cold concurrency-8 gate once | completed; hung at the single gate before the ladder was extended, so the narrow-fence variant was closed without an implementation | Result: `update_thread_join_begin` 29 with 28 returns and `graph_update_begin` 29 with 28 returns, with the guard-disabled and barrier-only warnings both attested and no gate completed. A handoff fence cannot order the racing pair, because it runs when `hold()` exits while `graph.update()` and `graph.replay()` are issued concurrently inside one generation hold; the full-device fence is a strict superset of the narrow `torch.npu.current_stream().synchronize()` from `vllm-ascend` PR `#6432`, so its failure closes the narrow variant as well. The barrier therefore cannot be justified as insurance, the mutex remains the only proven serialization, and ordered update stays the only mechanism that removes the third concurrent submitter. The `off` scope is not shipped either way. |
| 910C-067 | handoff commit containing this row; SGLang `e4d18390a`; Omni branch `qwen3-asr-guard-scope`; no server edit | Test whether the encoder's device submissions are a necessary participant in the hang, which `910C-065` inferred but never isolated | Three fresh `threaded + scope=off` processes at the `910C-013` profile, each first warming the encoder stage cache with one concurrency-1 pass over the 70 pinned inputs and then running the explicit cold concurrency-8 gate over the same inputs | completed; the encoder's device submissions are a necessary participant, so the attribution is a controlled result rather than an inference | Result: every warm pass completed and all three gates passed 70/70 at p95 0.52-0.53 s with zero encoder batches started in every gate phase, against a hang at the same profile in `910C-065` with the encoder active. Cache keys are `namespace:audio_fingerprint`, so the gate phase attached cached embeddings and queued no encoder work. The result also corrects one earlier reading: with the encoder silent the threaded path still submits from two host threads and is live, so two concurrent submitters are not sufficient to hang and the unsynchronized update/replay pair is not by itself the cause. The load-bearing variable is the number of concurrent host submitters rather than encoder device volume, since `910C-024B` ran a submitting encoder serialized by the guard and passed. |
| 910C-068 | handoff commit containing this row; SGLang `e4d18390a`; Omni branch `qwen3-asr-guard-scope`; no server edit | Find what the helper thread is blocked in below our own code, which every previous hang arm lost because it force-cleaned the process | Reproduce `910C-065` (cold encoder cache, encoder active) with `ASCEND_GLOBAL_LOG_TO_FILE=1`, `ASCEND_GLOBAL_LOG_TO_STDOUT=0`, `ASCEND_GLOBAL_LOG_LEVEL=1`, then sample the live process before the `910C-065` bounded forced cleanup: backtrace, the pid's plog, `/proc/<pid>/task/*/wchan`, `npu-smi`, and `msnpureport` in its own documented syntax | completed; reproduced, and the stall is now localized to the graph update path at the device layer | The stall is a device-side wait rather than a host lock: the update thread sits in `graph_task_update_end` with `wchan=eventfd_read`, waiting for a device completion signal, and `npu-smi` reads AI Core and Aicore at 0 percent while the process is stalled, so nothing is executing. plog's last visible entry is the encoder's `AllocTaskAndSendStars` for a `KERNEL_AICORE` task, so device work was still being submitted as the stall began. Three qualifications travel with this: `gdb` is absent, so the capture is `py-spy` plus `/proc`, that is a Python frame and a wait channel rather than a native backtrace, and a later arm must produce native frames before this is quoted as one; the silent indefinite wait on an idle device fits a submitted task whose dependency is never satisfied, the `vllm-ascend` `#4233` shape, more closely than a task the hardware rejected, which would normally raise or time out, so the "hardware scheduler refused the task" reading is not established by this evidence; and under INFO the plog ring buffer rotates in about four minutes, so the tail is not guaranteed to be the stall instant and a repeat should stream-copy plog while the run proceeds. `msnpureport` is not installed, so no device-side log exists for this run. Cleanup left a residual holder from an earlier experiment taking about 9.6 GiB, which must be resolved before the next arm. |
| 910C-069 | handoff commit containing this row; SGLang `e4d18390a`; Omni branch `qwen3-asr-guard-scope`; no server edit | Test whether the encoder sharing the default submission stream with decode graph replay and update is the mechanism, since CUDA gives the encoder a private stream and NPU does not | Two arms at the `910C-013` profile with `threaded` update, `scope=off` and a cold encoder cache: arm P sets `SGLANG_OMNI_NPU_ENCODER_BATCH_SYNC=1` (shared stream, worker blocked until that stream drains after each batch), arm Q sets `SGLANG_OMNI_NPU_ENCODER_PRIVATE_STREAM=1` (CUDA-parity private stream). Fresh processes, explicit cold concurrency-8 gate; stop an arm at its first hang and treat three completed gates as live | completed; arm P hung at Gate 1 and arm Q completed three gates, so the shared submission lane is the mechanism | Both arms attested their warning line. The prediction recorded before the run, that arm P would be live, was wrong, and the reason matters: `current_stream().synchronize()` drains the encoder worker only after a batch, so the encoder still submits concurrently with the generator's replay and update inside every batch, while the guard removes that concurrency. Arm Q keeps the same concurrency and changes only which queue the encoder's work occupies. The mechanism statement is therefore: two concurrent submitters on one stream are safe (`910C-067`), three on one stream hang (`910C-065`, `910C-066`, arm P), and moving the encoder off that stream with all three still running removes the hang (arm Q). The handoff-barrier family is closed from two directions, a full-device fence at hold exit and a per-batch stream drain. The private stream is no longer only a CUDA-parity tidy-up: it is a candidate replacement for the guard, because it removes the hazard while keeping encoder and generation overlapping, and validating that requires correctness and performance at the accelerated profile rather than liveness alone. The switch as it stands is still not shippable, since it changes three things at once (the stream, an active `synchronize_batch()`, and the embedding consumer registration) and only liveness was measured. This run also left HBM at 63 percent, so the device was not returned to its idle baseline and that must be resolved before the next arm. |
| 910C-070 | handoff commit containing this row; SGLang `e4d18390a`; Omni branch `qwen3-asr-guard-scope`; no server edit | Decide whether the private stream can replace the guard, since `910C-069` measured only liveness at the `910C-013` profile and never correctness or performance at the accelerated profile | Two arms at the M1c profile with `threaded` update: arm A sets `SGLANG_OMNI_NPU_ENCODER_PRIVATE_STREAM=1` with `scope=off` and no batch fence; arm B is the retained reference, the default `forward` guard with the private-stream switch unset. Each arm runs one fresh 140-request correctness process, then three fresh `exact10` C70 processes with the packaged harness and warm-up | completed; arm A won on correctness and performance, so the private stream replaces the guard | Both arms passed the correctness gate at WER 0.0179 with zero garbled output. Arm A's three C70 repeats measured p95 1.350-1.474 s and 59.65-62.88 req/s against arm B's 1.543-1.705 s and 52.52-55.54: the private stream is not a liveness repair that costs throughput, it is faster than the guard it replaces. Arm A attested the private-stream warning and a null `device_execution_guard`; arm B attested a non-null guard and no such warning. Both hygiene items closed on the box: `torch_npu` does expose `record_stream`, so the cross-stream handoff registers its consumer rather than skipping, and what remains is deleting the environment gate rather than promoting it. The guard is therefore no longer the shipping repair for this hazard; its justification becomes historical in the same sense ordered input update made it historical. Follow-up is a real change rather than a switch: make the NPU encoder stream unconditional, delete the environment gate, and restate both pull requests so the omni change reads as a stream fix rather than a mutex. |
| 910C-071 | handoff commit containing this row; SGLang `qwen3-asr-ascend-full` at `891a70626`; Omni `qwen3-asr-ascend-full` at `d58afeda`; no server edit | Qualify the guard-free implementation on hardware, since the guard and ordered input update were deleted and `910C-070` arm A only expressed that configuration through a diagnostic switch on a tree that still carried the guard | Three phases: (1) the `910C-013` profile with a cold encoder cache, three fresh processes each running the explicit cold concurrency-8 gate; (2) one fresh M1c process through the 140-request correctness workload; (3) three fresh M1c processes each running one `exact10` C70 measurement with the packaged harness and warm-up at `torch_compile_max_bs=2` | authorized after a clean idle NPU baseline; pending | The guard and fence environment variables no longer exist, so nothing about them is set, and model-info no longer carries `device_execution_guard`. Attest the deletion before starting: `rg -n "execution_guard" sglang_omni` and `rg -n "SGLANG_OMNI_NPU_EXECUTION_GUARD_SCOPE\|SGLANG_OMNI_NPU_GUARD_COMPLETION_FENCE" sglang_omni tests` must both return nothing, and `rg -n "SGLANG_NPU_GRAPH_INPUT_UPDATE_MODE" python` must return nothing in the SGLang checkout. Bounded forced cleanup as in `910C-065`, which applies to phase 1 in particular. Readings: three live gates, correctness in band with zero garbled output, and three C70 repeats within about p95 1.35-1.50 s at or above about 59 req/s qualify the retirement, after which the roadmap reference table can be refreshed from this run; any hang means the retirement is not qualified and the guard must be restored from history rather than reintroduced as a switch; a correctness failure, especially garbled output, points at the cross-stream embedding handoff and also blocks the retirement; a material performance regression against the `910C-054` A1 baseline means the deletion disturbed more than the guard and must be diagnosed before any reference number is published. Both candidate branches were then cut back to the behaviour changes - the decode-graph stage logging, the process-scoped compile diagnostics, the prefill capture-only gate, the encoder graph counters and model_info payload, and the explicit compile-bucket list are gone - so the counter readback no longer exists, the phases must instead confirm that no `captured encoder layer-stack graph` line appears and read the upstream replay record, and the `910C-070` arm A and `910C-054` M1c performance numbers are not reproducible on these heads; the performance gate is the `910C-054` A1 baseline, which itself ran an encoder graph this head cannot build. The SGLang candidate was subsequently rewritten from `dfaf5e50c` to `891a70626`, removing the stateful decode-attention graph break and the graph-context/compile-bucket work while retaining only the fused-op compile boundary and shared compile-safe context. The `910C-034`/`910C-035` runs never isolated the remaining fused-op dispatch changes cleanly, and the `910C-054` correctness result belonged to the removed graph-break code, so phase 2 on `891a70626` is fresh evidence rather than a reproduction of that result. |
