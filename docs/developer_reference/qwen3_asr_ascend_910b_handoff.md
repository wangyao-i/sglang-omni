...
| 910C-068 | handoff commit containing this row; SGLang `e4d18390a`; Omni branch `qwen3-asr-guard-scope`; no server edit | Find what the helper thread is blocked in below our own code, which every previous hang arm lost because it force-cleaned the process | Step 0: look for plog left by the `910C-065`/`910C-066` hangs. Then reproduce `910C-065` (cold encoder cache, encoder active) with `ASCEND_GLOBAL_LOG_TO_FILE=1`, `ASCEND_GLOBAL_LOG_TO_STDOUT=0`, `ASCEND_GLOBAL_LOG_LEVEL=1`, and when the gate stalls, sample the live process before cleanup: native backtrace via `gdb -p <pid> -batch -ex "thread apply all bt"`, the pid's plog tail, `/proc/<pid>/task/*/wchan`, `npu-smi info` and `-t usages`, and `msnpureport` only in the syntax its own `-h` documents | authorized after a clean idle NPU baseline; pending; a non-reproduction is a valid outcome | The collection window is bounded to ten minutes and runs before the `910C-065` bounded forced cleanup, which is still authorized and whose cleanup record is still required; do not kill the server to shorten the window. Use log level 1, not 0, because the extra volume perturbs the timing the hang depends on, and do not raise the level to retry inside this arm. Record the plog path actually found rather than assuming one. If `gdb` is absent, substitute `py-spy dump` and say so explicitly, since native frames are the primary artifact. Return text only: plog tail, native frames around the blocking call, wait channels, device snapshots, reproduction status, cleanup record; keep raw logs and core files on the isolated server. Native frames naming a blocking ACL call inside the graph update path make the finding upstream-fileable; a wait tied to another stream's outstanding work means the multi-stream interaction is the mechanism; frames stopping in process-local state with no device wait mean a host-side interlock. |
