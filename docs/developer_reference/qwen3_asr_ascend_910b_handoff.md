...
| 910C-066 | handoff commit containing this row; SGLang `e4d18390a`; Omni branch `qwen3-asr-guard-scope`; no server edit | Decide whether a device barrier replaces the FIFO mutex, tested where the hazard exists rather than at a profile that never hangs | Barrier-only mode is already in place: `scope=off` plus `SGLANG_OMNI_NPU_GUARD_COMPLETION_FENCE` builds `FairDeviceExecutionGuard(serialize=False)`, bypassing the ticket lock while the existing full-device fence runs at every handoff. First one fresh process at the `910C-013` profile with `threaded` update running the explicit cold concurrency-8 gate once; only if it completes, the full ladder of three fresh gates plus one fourth-process 140 correctness and exact10 C70 | authorized after a clean idle NPU baseline; pending; run the single strong-barrier gate first | A handoff fence cannot order the racing pair, because it runs when `hold()` exits while `graph.update()` and `graph.replay()` are issued concurrently inside one generation hold; the full-device fence is a strict superset of the narrow `torch.npu.current_stream().synchronize()` from `vllm-ascend` PR `#6432`, so a hang with the strong barrier closes the narrow variant without an implementation. Attest from startup logs and model-info that the mutex is disabled, the barrier is enabled, and per-label guard statistics are empty. Bounded forced cleanup and the `910C-065` cleanup record are required on a hang. If the single gate hangs, the barrier cannot be justified as insurance and the mutex remains the only proven serialization, leaving ordered alone (`910C-064`) versus ordered plus the mutex; if it completes and the full ladder completes, implement the narrow fence and measure its cost against the `910C-064` guard-off reference before considering it. The `off` scope is not shipped either way. |
| 910C-067 | handoff commit containing this row; SGLang `e4d18390a`; Omni branch `qwen3-asr-guard-scope`; no server edit | Test whether the encoder's device submissions are a necessary participant in the hang, which `910C-065` inferred but never isolated | Three fresh `threaded + scope=off` processes at the `910C-013` profile, each first warming the encoder stage cache with one concurrency-1 pass over the 70 pinned inputs and then running the explicit cold concurrency-8 gate over the same inputs | authorized after a clean idle NPU baseline; pending | Cache keys are `namespace:audio_fingerprint`, so the gate phase attaches cached embeddings and queues no encoder work; each process must attest zero encoder batches started during its gate phase or it does not count. Report the warm pass separately: a hanging warm pass is that process's result and does not establish the profile. Bounded forced cleanup as in `910C-065`. All three gates completing means the encoder's submissions are necessary and the mechanism statement stands; any gate hanging with zero encoder batches means the encoder is not a necessary participant and the encoder-versus-decode-graph attribution is withdrawn. Keep every profile setting fixed; do not retry with the guard re-enabled or with ordered mode. |
