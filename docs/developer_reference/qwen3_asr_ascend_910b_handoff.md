...
| 910C-061 | handoff commit containing this row; SGLang `e4d18390a`; Omni `21e755be`; no server edit | Qualify ordered graph input update under the retained `forward` guard, which no accuracy-bearing run has yet measured, before proposing it upstream | Read-only 059 Arm O field check, then one fresh `ordered + scope=forward` process at the fixed M1c profile: batch one, cold conc8, 140 correctness, conditional exact10 C70 | authorized after a clean idle NPU baseline; pending | Require the resolved guard scope to be `forward`, the compile list to be exactly `[1,2,70]`, and ordered update mode attested at startup. Stop before C70 unless every request completes with 140-request WER inside the qualified 0.016 band, zero garbled, zero failure/timeout/missing/duplicate/unexpected, and zero eager fallbacks. Accept ordered mode for upstream only if C70 accuracy matches M1c and p95/throughput do not regress against 1.442 s and 59.88 req/s, with full cleanup to baseline HBM. Do not test another scope, ordering, token cap, bucket set, or memory fraction. |
