...
| 910C-057 | Omni candidate `qwen3-asr-guard-scope` through compatibility fix `fb5110a9`; SGLang candidate `qwen3-asr-perf-next` through `b03400e5a` | Independently remove the stateful decode-attention compile break and narrow the FIFO guard from the whole generation forward to exact graph dispatch | Serial AC explicit-state attention, GS graph-scoped guard, and conditional AG composition; each has correctness before C70 performance | reauthorized after preflight-only failure; fixed M1c `[1,2,70]` baseline | Initial Omni `ab688d97` stopped before hardware because two pipeline tests failed: the new test lacked its context-manager import and the implementation wrote guard observability attributes even when no guard existed. `fb5110a9` restores the legacy no-guard contract without changing guarded behavior. Repeat the complete preflight, then apply the original AC/GS/AG gates. |
