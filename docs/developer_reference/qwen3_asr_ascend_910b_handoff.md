...
| 910C-071 | handoff commit containing this row; SGLang `qwen3-asr-ascend-full` at `891a70626`; Omni `qwen3-asr-ascend-full` at `d58afeda`; no server edit | Qualify the guard-free implementation on hardware, since the guard and ordered input update were deleted and `910C-070` arm A only expressed that configuration through a diagnostic switch on a tree that still carried the guard | Three phases: (1) the `910C-013` profile with a cold encoder cache, three fresh processes each running the explicit cold concurrency-8 gate; (2) one fresh M1c process through the 140-request correctness workload; (3) three fresh M1c processes each running one `exact10` C70 measurement with the packaged harness and warm-up at `torch_compile_max_bs=2` | completed; passed on the task-pinned candidate heads | See the result note below the table. |

**`910C-071` result.** The server report returned the three code-identity checks
with no output: no `execution_guard` in `sglang_omni/`, no guard or fence
environment variables in `sglang_omni/` plus `tests/`, and no
`SGLANG_NPU_GRAPH_INPUT_UPDATE_MODE` in `sglang/python/`. Phase 1 passed 3/3 cold
concurrency-8 gates with zero failures and zero timeouts. Phase 2 passed the
140-request correctness gate at WER `0.0181` with zero garbled outputs. Phase 3
passed 3/3 fresh-process C70 measurements at p95 `1.460-1.623` s and
`58.47-59.16` requests/s. This is better than the predeclared `910C-054` A1
floor for this task (`p95` about `5.98` s and about `35` requests/s), so the
guard retirement is qualified on the task-pinned heads. The report did not
restate the two repository hashes, so this row preserves the identities pinned
by the task rather than inferring a different run. The Omni branch was later
advanced by `f89d565f` and `872e5502` to incorporate stream-helper review
changes; those commits are behavior-preserving in intent but are outside this
exact-head hardware record and require separate re-attestation if treated as the
shipping revision.
