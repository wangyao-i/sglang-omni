...
| 910C-058 | Omni implementation `f9aeabad` plus this handoff; SGLang `4db662590` | Guard the complete SGLang raw model execution region instead of graph execute alone | Serial focused/full tests, cold concurrency 8, 140 correctness, then conditional fixed-M1c C70 | authorized; pending server run | Set `SGLANG_OMNI_NPU_EXECUTION_GUARD_SCOPE=model`; AC diagnostics must be absent. Require correct output, all graph paths exercised, balanced labeled guard statistics, final outstanding zero, drain, and clean NPU teardown. Stop on first failure. |
