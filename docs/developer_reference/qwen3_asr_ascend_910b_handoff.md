...
| 910C-057 | Omni candidate `qwen3-asr-guard-scope` through compatibility fix `fb5110a9`; SGLang candidate `qwen3-asr-perf-next` through `27b246532` | Independently remove the stateful decode-attention compile break and narrow the FIFO guard from the whole generation forward to exact graph dispatch | Serial AC explicit-state attention, GS graph-scoped guard, and conditional AG composition; each has correctness before C70 performance | AC startup failed before requests; corrected and reauthorized; fixed M1c `[1,2,70]` baseline | After the test-only failure was fixed, AC exposed a production ABI error: graph capture metadata intentionally has no `seq_lens_cpu_int`. SGLang `27b246532` now carries required `ForwardBatch.seq_lens`, the existing device graph-input buffer updated during replay, and adds a graph-metadata regression test. Repeat the complete preflight and AC from a fresh process. AC failure does not block independent GS; retain all original correctness, deadlock, fallback, and cleanup gates. |
