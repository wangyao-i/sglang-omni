...
| 910C-069 | handoff commit containing this row; SGLang `e4d18390a`; Omni branch `qwen3-asr-guard-scope`; no server edit | Test whether the encoder sharing the default submission stream with decode graph replay and update is the mechanism, since CUDA gives the encoder a private stream and NPU does not | Two arms at the `910C-013` profile with `threaded` update, `scope=off` and a cold encoder cache: arm P sets `SGLANG_OMNI_NPU_ENCODER_BATCH_SYNC=1` (shared stream, worker blocked until that stream drains after each batch), arm Q sets `SGLANG_OMNI_NPU_ENCODER_PRIVATE_STREAM=1` (CUDA-parity private stream). Fresh processes, explicit cold concurrency-8 gate; stop an arm at its first hang and treat three completed gates as live | authorized after a clean idle NPU baseline; pending | Setting both selects arm Q and logs that the fence was ignored. Attest the matching warning line per arm plus code identity with `rg -n "SGLANG_OMNI_NPU_ENCODER_PRIVATE_STREAM|SGLANG_OMNI_NPU_ENCODER_BATCH_SYNC" sglang_omni/models/qwen3_asr/encoder_service.py`. Arm P exists to hold "the host does not run ahead" constant while the stream layout changes; without it a live arm-Q result cannot be attributed to the stream, because that arm changes three things at once (the stream, an active `synchronize_batch()`, and the embedding consumer registration). Readings: live/live means stream sharing is not the mechanism and the host throttle is sufficient, matching the `910C-024B` guard result; hang/hang means the hazard sits below the host submission layer, consistent with the never-signalled completion in `910C-068`; hang/live means stream sharing is the mechanism and the CUDA-parity layout removes it, a real defect with a real fix that still needs its own validation; live/hang is not expected and must be reported and stopped rather than read as either outcome. Bounded forced cleanup as in `910C-065`. No outcome authorises shipping either switch, and no outcome changes the retained decision: the guard stays unconditional and ordered update remains the only mechanism that removes the third concurrent submitter. |
