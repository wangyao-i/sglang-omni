...
| 910C-059 | Omni `99aabb26`; SGLang `e4d18390a`; no server edit | Replace the NPU backend's Python update-thread/join overlap with opt-in same-thread ordered `NPUGraph.update()` then `replay()` | Serial focused/full tests; Arm O with qualified forward guard; conditional fresh graph-scope arm under the fixed M1c profile | ordered liveness passed; graph scope rejected | Tests passed (SGLang 16; Omni guard 4, pipeline 42, Qwen3-ASR 606 with 3 skipped). Arm O completed cold conc8 70/70, correctness 140/140, and C70 700/700; C70 throughput was 52.28/s, 5,083 ordered pairs completed, update-thread markers were zero, and guard outstanding was zero. Arm G also drained, but its 140-request WER was 0.0778 versus the qualified approximately 0.016 band; C70 p95 was 1.81 s and throughput 51.66/s versus M1c 1.442 s and 59.88/s. Arm G cleanup left HBM at 22%. Ordered execution closes the helper-thread/join liveness defect, but graph scope fails accuracy, performance, and cleanup qualification. Do not promote graph scope. |
...
| 910C-060 | SGLang `e4d18390a`; Omni `21e755be`; no server edit | Test the only remaining coherent narrowed boundary after ordered update removed the prior model-scope join stall | One fresh `ordered + scope=model` process using the fixed M1c profile; 140 correctness then conditional exact10 C70 | rejected; failed the 140-request accuracy gate before C70 | Live and deadlock-free: correctness 140/140, zero garbled output, 4,707 ordered update/replay pairs, zero update-thread markers, final guard `next=serving=5324, outstanding=0`. Rejected on the predeclared gates: the 140-request WER was 0.0778 against the qualified 0.0161 band, so the run should have stopped before C70; C70 p95 1.75 s and 52.79 req/s did not materially beat M1c (1.442 s, 59.88 req/s); teardown HBM was not confirmed. `910C-059` Arm G returned the same 0.0778, so both narrowed scopes share one accuracy signature. Retain the broad `forward` scope; guard narrowing is closed. |
...
| 910C-063 | handoff commit containing this row; SGLang `e4d18390a`; Omni branch `qwen3-asr-guard-scope`; no server edit | Discriminate whether the NPU execution guard is load-bearing or only orders two device producers, following the `vllm-ascend` `#4233` precedent | One fresh `ordered + scope=off` process at the fixed M1c profile: batch one, cold conc8, 140 correctness, conditional exact10 C70 | authorized after `910C-062` completes and after a clean idle NPU baseline; pending | Require `off` in the accepted scope set at the code commit, the guard-disabled startup warning, and a model-info attestation that no guard is installed; require `git diff --name-only <that commit> HEAD` to list only `docs/` paths. Stop before C70 unless the 140-request WER is inside the qualified band with zero garbled/failure/fallback. If the run completes correctly and live, the guard is not load-bearing and a later task may replace it with a stream event between the encoder stream and the generation graph. If it hangs or fails, the guard is load-bearing and the review request to drop or condition it must be declined with this evidence. Do not retry with another scope, ordering, token cap, bucket set, or memory fraction. |
