...
| 910C-070 | handoff commit containing this row; SGLang `e4d18390a`; Omni branch `qwen3-asr-guard-scope`; no server edit | Decide whether the private stream can replace the guard, since `910C-069` measured only liveness at the `910C-013` profile and never correctness or performance at the accelerated profile | Two arms at the M1c profile with `threaded` update: arm A sets `SGLANG_OMNI_NPU_ENCODER_PRIVATE_STREAM=1` with `scope=off` and no batch fence; arm B is the retained reference, the default `forward` guard with the private-stream switch unset. Each arm runs one fresh 140-request correctness process, then three fresh `exact10` C70 processes with the packaged harness and warm-up | authorized after a clean idle NPU baseline; pending | Compare only A against B on this session; historical M1c numbers come from a different day and process lifecycle. Stop an arm at its first hang, timeout, accuracy failure, OOM, device error or cleanup failure, with bounded forced cleanup as in `910C-065`. Attest the private-stream warning plus `device_execution_guard: null` in arm A, a non-null guard and no such warning in arm B, plus code identity with `rg -n "SGLANG_OMNI_NPU_ENCODER_PRIVATE_STREAM" sglang_omni/models/qwen3_asr/encoder_service.py`. Readings: arm A in band with zero garbled output and three C70 repeats not materially worse than arm B means the private stream is a viable replacement and the guard's justification becomes historical, with a follow-up that makes the NPU stream unconditional and removes the environment gate; arm A failing correctness, especially with garbled output, means the private stream is not sufficient at the accelerated profile and the guard stays, which would point at the cross-stream embedding handoff rather than at device scheduling; arm A passing correctness but measuring materially worse than arm B means the private stream costs more than the guard buys; a hang in either arm contradicts `910C-069` and must be reported rather than read either way. Two hygiene items must close before the private stream is proposed: the `hasattr`-guarded `record_stream` in `attach_embedding` would skip silently if `torch_npu` does not expose it, leaving the allocator side of the handoff unverified, and shipping means making the behaviour unconditional instead of keeping a second switch. |
