...
| 910C-066 | handoff commit containing this row; SGLang `e4d18390a`; Omni branch `qwen3-asr-guard-scope`; no server edit | Decide whether a narrow device barrier replaces the FIFO mutex, tested where the hazard exists rather than at a profile that never hangs | One local change so `scope=off` plus `SGLANG_OMNI_NPU_GUARD_COMPLETION_FENCE` yields a barrier without the ticket lock, then the `910C-013` profile with `threaded` update: three fresh processes each executing the explicit cold concurrency-8 gate, then one fourth-process 140 correctness and exact10 C70 | authorized after a clean idle NPU baseline; pending; requires the local barrier change first | Use `torch.npu.current_stream().synchronize()` at each handoff, mirroring `vllm-ascend` PR `#6432`, not the full-device `device_module.synchronize()`. Attest from startup logs and model-info that the mutex is disabled, the barrier is enabled, and per-label guard statistics are empty. Bounded forced cleanup and the `910C-065` cleanup record are required on a hang. All three gates plus the fourth process completing means the barrier suppresses the known mechanism and is a valid substitute, leaving the choice between ordered alone and ordered plus a barrier to its measured cost against the `910C-064` guard-off reference; any hang means the barrier does not address the mechanism and the mutex remains the only proven serialization. The `off` scope is not shipped either way. |
