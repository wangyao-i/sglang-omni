...
| 910C-060 | handoff commit containing this row; SGLang `e4d18390a`; no server edit | Test the only remaining coherent narrowed boundary after ordered update removed the prior model-scope join stall | One fresh `ordered + scope=model` process using the fixed M1c profile; 140 correctness then conditional exact10 C70 | authorized after clean NPU baseline; pending | Require pre-start HBM at the idle baseline with no holder. Run 140 correctness and stop unless WER is in the qualified band with zero garbled/failure/fallback counts. Only then run exact10 C70 and return complete accuracy, latency, throughput, graph, guard, and cleanup metrics. Compare with M1c and 910C-059 O/G. If model scope is incorrect, hangs, fails cleanup, or does not materially beat M1c, close guard narrowing and retain broad forward scope. Do not test another scope. |
