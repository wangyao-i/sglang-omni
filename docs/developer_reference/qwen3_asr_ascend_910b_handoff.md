...
| 910C-056B | handoff commit containing this row; Omni `fcc2fb64`; SGLang `edc504ff6` | Separate KV right-sizing from compile coverage, then spend recovered HBM on all decode buckets | Serial K0 `[1,2]`, F13 all 13 buckets, conditional F8 on F13 capture OOM, then three fresh-process C70 repeats of the winner | authorized; pending server run | All arms pin 32,768 KV tokens and the fully enabled ALL profile. K0 attributes KV-only gain; F13 tests full compile coverage. Existing observability supplies one bounded performance attribution; no source/package edits, soak, realtime, or unlisted bucket retries are authorized. |
| 910C-057 | Omni candidate `qwen3-asr-guard-scope` at the handoff commit containing this row (code through `f24d6220`); SGLang candidate `qwen3-asr-perf-next` through `b03400e5a` | Independently remove the stateful decode-attention compile break and narrow the FIFO guard from the whole generation forward to exact graph dispatch | After `910C-056B`, serial AC explicit-state attention, GS graph-scoped guard, and conditional AG composition; each has correctness before C70 performance | prepared; blocked only on selecting the successful `910C-056B` compile list | Both changes are opt-in and default-safe. AC makes ForwardBatch/KV tensor state and mutation explicit to Dynamo. GS retains encoder serialization but guards only prefill/decode graph execute and exposes labeled wait/hold totals. Cold-input deadlock, zero eager/fallback, WER, request accounting, balanced guard, and cleanup are hard gates. |
